...
import sa_oa.plantgl.all as pgl
...
def surfaceByShape(sc):#aka surfaces
  surfbysh = {}
  for i in sc:
    surfbysh[i.id] = pgl.surface(i.geometry) #where in sa_oa.plantgl ??
  return surfbysh
...
def surfPerLeaf(sc):
  ...
  for sh in sc:
    ...
    for i in range(d.indexListSize()):
      ...
      surf = pgl.surface (d.pointAt(i,0),d.pointAt(i,1),d.pointAt(i,2)) #where in sa_oa.plantgl ??
      res2 += [surf]
    lcenter /= len( res2 )
    lsurf=sum( res2 )
    res+=[ ( lcenter, lsurf ) ]
