...
def set_vtk_strings(db):
	...
	for cid in mesh.wisps(3):
		...
		xvals=[position[i][0] for i in cell_pids[cid]]
		yvals=[position[i][1] for i in cell_pids[cid]]
		zvals=[position[i][2] for i in cell_pids[cid]]
		xlen=len(cell_pids[cid])
	...
	for wid in mesh.wisps(2):
		...
		for cid in mesh.regions(2,wid):
			...
			for i,pid in enumerate(pids):
				...
				if pid not in moved_pid_vecs.iterkeys():
					vec = get_ofset_vec(position[xpid] for xpid in p_pids)
				else:
					vec = moved_pid_vecs[pid]
