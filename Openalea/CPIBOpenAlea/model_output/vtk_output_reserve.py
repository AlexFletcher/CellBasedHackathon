...
def set_vtk_strings(db):
	...
	wall_pid_refs=[]
	print 'setting walls'
	for i,wid in enumerate(mesh.wisps(2)):
		pids = ordered_pids(mesh, wid)

		for xpid in pids:
			pt = position[xpid]
			wall_ps.append("%s %s %s " % (str(pt[0]),str(pt[1]),str(pt[2])))
			wall_pid_refs.append(xpid)
			wall_co +=1
				
		wall_os.append(wall_co)
		wid_trans[i]=wid


	wall_ts = ' '.join(["7" for num in xrange(len(wid_trans))])
	wall_cs=' '.join([str(num) for num in xrange(wall_co)])
	swall_ps=''.join([str(val) for val in wall_ps])
	swall_os=' '.join([str(val) for val in wall_os])
	wall_dict={'position_string':swall_ps,'connect_string':wall_cs,\
			'offset_string':swall_os,'type_string':wall_ts,'n_cells':str(len(wid_trans)),'n_points':str(wall_co),'cid_trans':wid_trans}
	print 'walls done'
	"""
	...
	for cid in mesh.wisps(3):
		cell_pts = [position[i] for i in cell_pids[cid]]
		xvals=[i[0] for i in cell_pts]
		yvals=[i[1] for i in cell_pts]
		xlen=len(pts)
		xy_centre=array([sum(xvals)/xlen,sum(yvals)/xlen])
		...
		for wid in mesh.borders(3,cid):
			...
			for i,pid in enumerate(pids):
				...
				vec = get_offset_vec([position[xpid] for xpid in p_pids],horiz_centre)
