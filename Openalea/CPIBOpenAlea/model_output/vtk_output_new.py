...
def set_vtk_strings(db):
	...
	widths=[cell_wall_width,cell_wall_width/2.0]
	...
	for wid in mesh.wisps(2):
		...
		wid_pos+=1
		offset_points=face_interior_offset_3d_vert(pts,widths)
		for cid in mesh.regions(2,wid):
			for pt in offset_points[0]:
				cell_ps += "%s %s %s " % (str(pt[0]),str(pt[1]),str(pt[2]))
			...
			for pt in offset_points[1]:
				edge_ps += "%s %s %s " % (str(pt[0]),str(pt[1]),str(pt[2]))
				edge_cs += "%s " % str(edge_pp)
				edge_co +=1
				edge_pp +=1
			edge_os+="%s " % str(edge_co)
			eid_trans[eid_pos]=15938
			eid_pos+=1					
