...
def flux_plot_2(db,iteration,dir_name,maxlength):
	mesh=get_mesh(db)
	graph=get_graph(db)
	...
	auxin_flux=db.get_property('auxin_flux')
	...
	for v in range (0,max(list(vindex.itervalues()))+1):
		...
		for wid,(eid1,eid2) in wall_decomp.iteritems():
			...
			if vindex[graph.source(eid1)]==vindex[graph.source(eid2)]:
				if flux>0:
					polygon = Polygon(edge_coords[eid2], True)
				else:
					polygon = Polygon(edge_coords[eid1], True)
		...
		for wid,(eid1,eid2) in wall_decomp.iteritems():
			...
			if vindex[graph.source(eid1)]==vindex[graph.source(eid2)]:
				if flux>0:
					...
					Flux_colours.append(auxin_flux[eid1])


		if len(Flux_colours)>0:
			p = PatchCollection(epat, cmap=matplotlib.cm.jet, alpha=1.,linewidths=(1,))
